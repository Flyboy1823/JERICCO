...
%linear search for upper and lower bounds in the data for a given stored
%angular momentum
for i=1:n
...
if(Commanded_Torque<TorqueBox_data(target_i_max,3)&&Commanded_Torque>TorqueBox_data(target_i_min,3))
    ...
    %if is not between the bounds return the upper bound if is commanded
    %torque is positive or the lower bound if is negative
